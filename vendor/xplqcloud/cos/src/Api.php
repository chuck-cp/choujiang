...
class Api {
   ...
   // private $endPoint = 'http://mfs.1818lao.com/rs-mfs_filesys/v1/';
//    private $endPoint = 'http://mfs.1818lao.com/rs-mfs_filesys/v1/mfsFileDealServiceWs/savePhoneFile/';
...
//        }
     ...
     * 生成token,用于移动端上传
     * */
    public function createReusableSignature($bucket){
        $expired = time() + self::EXPIRED_SECONDS;
        return $this->auth->createReusableSignature($expired, $bucket);
    }
    /**
     * 下载文件
     * @param  string  $bucket  bucket名称
   ...
   /*
    ...
    public function getDownloadUrl($bucket, $fpath, $expireAfterSecs) {
        ...
        $accessUrl = "http://$bucket-$appId.mfs.1818lao.com$fpath?sign=$signature";
        $sourceUrl = "http://$bucket-$appId.cos${region}.1818lao.com$fpath?sign=$signature";
        $url = "http://$region.mfs.1818lao.com/rs-mfs_filesys/v1/${appId}${fpath}?sign=$signature";
        ...
        http://f0.1818lao.com/img/luckdraw/1908/1566889754.jpeg
        return array(
            'code' => 0,
            'message' => 'SUCCESS',
            'data' => array(
